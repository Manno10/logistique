...
function bindEvents(){
  ...
  $('syncBtn').onclick=async()=>{await synchroniserScans();showMessage('Synchronisation terminée.','ok')};$('exportCsvBtn').onclick=exportCsv;$('exportJsonBtn').onclick=exportJson;$('clearBtn').onclick=clearScans;
...
async function saveScan(){
  ...
  await add(SCANS_STORE,record);showMessage('Scan enregistré sur le téléphone.','ok');if(navigator.onLine){await synchroniserScans();}await refreshDashboard();await refreshRecent();updateQueue();setTimeout(resetForm,1800)
}
...
function jsonp(action,params={}){
  return new Promise((resolve,reject)=>{
    if(!API_URL || API_URL.includes('REMPLACER_PAR_URL')){
      reject(new Error('URL Apps Script non configurée.'));
      return;
    }

    const callback='elms_cb_'+Date.now()+'_'+Math.floor(Math.random()*100000);
...
async function updateScan(localId,changes){
  return new Promise((resolve,reject)=>{
    const t=db.transaction(SCANS_STORE,'readwrite');
...
async function synchroniserScans(){
  if(!navigator.onLine) return;

  const scans=await getAll(SCANS_STORE);
  const pending=scans.filter(scan=>!scan.synced);

  for(const scan of pending){
    try{
      const response=await jsonp('save_scan',{
        scanId:scan.scanId,
        timestamp:scan.timestamp,
        centreUid:scan.centreUid,
        qrValue:scan.qrValue,
        nomCentre:scan.nomCentre,
        departement:scan.departement,
        commune:scan.commune,
        section:scan.section,
        latitudeReference:scan.latitudeReference ?? '',
        longitudeReference:scan.longitudeReference ?? '',
        latitudeScan:scan.latitudeScan ?? '',
        longitudeScan:scan.longitudeScan ?? '',
        precisionGps_m:scan.precisionGps_m ?? '',
        distanceCentre_m:scan.distanceCentre_m ?? '',
        verificationGps:scan.verificationGps,
        typeOperation:scan.typeOperation,
        livraisonComplete:scan.livraisonComplete,
        nomReceptionnaire:scan.nomReceptionnaire,
        telephoneReceptionnaire:scan.telephoneReceptionnaire,
        observation:scan.observation,
        appareil:scan.appareil
      });

      if(response.ok){
        await updateScan(scan.localId,{
          synced:true,
      ...
      console.warn('Synchronisation interrompue :',error);
      break;
    }
  }
